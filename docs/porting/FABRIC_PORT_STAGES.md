...
- currently landed core slices:
  ...
  - `ScriptLoader` section-node fallback now restores the better retained section-versus-statement diagnostic, plain conditions no longer silently parse as section headers, and stopping statements now emit unreachable-code warnings behind script-level warning suppression
  ...
  - `ClassInfo` / `Classes` now close codename, literal-pattern, supertype-lookup, and class-info ordering behavior used by parser/runtime compatibility paths

...
- currently landed core slices:
  ...
  - exact upstream `feed [the] %players% [by %-number% [beef[s]]]` and `make %livingentities% invisible/visible` effect forms are now registered and live-verified on the active Fabric runtime
  ...
  - `./gradlew runGameTest --rerun-tasks` passed on 2026-03-08 with `220 / 220`
...
- current real-script Fabric GameTest suite is green at `220 / 220`
...
- 2026-03-08: `./gradlew test --tests ch.njol.skript.lang.function.FunctionCoreCompatibilityTest --tests ch.njol.skript.lang.function.FunctionImplementationCompatibilityTest --tests ch.njol.skript.lang.function.FunctionCallCompatibilityTest --tests ch.njol.skript.registrations.ClassesCompatibilityTest --tests ch.njol.skript.lang.SkriptParserRegistryTest --tests ch.njol.skript.patterns.PatternCompilerCompatibilityTest --rerun-tasks`, `./gradlew runGameTest --rerun-tasks`, and `./gradlew build --rerun-tasks` all passed after restoring dynamic-function validation cache discrimination, recursive `Classes.clone(...)` array isolation, and `SkriptParser.ParseResult.source`; the active suite stayed at `230 / 230`.
- 2026-03-08: `./gradlew test --tests ch.njol.skript.ScriptLoaderCompatibilityTest --tests ch.njol.skript.lang.function.FunctionCoreCompatibilityTest --tests ch.njol.skript.lang.function.FunctionImplementationCompatibilityTest --tests ch.njol.skript.lang.function.FunctionCallCompatibilityTest --tests ch.njol.skript.lang.VariableCompatibilityTest --rerun-tasks`, `./gradlew runGameTest --rerun-tasks`, and `./gradlew build --rerun-tasks` all passed after restoring local function-first registry fallback, section-scope hint clearing, effect-candidate section ownership reset, and retained section diagnostics on statement fallback success; the active suite stayed at `230 / 230`.

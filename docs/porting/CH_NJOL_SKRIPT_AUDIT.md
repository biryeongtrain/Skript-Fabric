...
- restored the missing legacy log-handler stack through `LogHandler`, `BlockingLogHandler`, `FilteringLogHandler`, `CountingLogHandler`, and `RetainingLogHandler`, and rewired `ParseLogHandler` plus `SkriptLogger` through that compatibility surface
- restored `patterns.Keyword` and upstream-style keyword prefiltering on the `SkriptPattern` compatibility path
- restored the legacy `variables.TypeHints` add/get/enter-scope/exit-scope/clear bridge over the active hint manager
- restored legacy parser/converter wrapper surfaces through `classes.Parser`, `classes.PatternedParser`, `classes.Converter`, and `registrations.Converters`

...
- restored dynamic-function validation cache discrimination in `DynamicFunctionReference`, so the cache now distinguishes concrete parameter expressions instead of only return-type tuples
- restored recursive array cloning in `Classes.clone(...)`, which closes keyed-function-argument array mutation leaks and improves upstream isolation semantics for array-backed values
- restored `SkriptParser.ParseResult.source` metadata on both `parseModern(...)` and `parseStatic(...)`
- deferred one adjacent loader fix instead of force-merging it: successful section-line statement fallback still needs a clean retained-section-log regression that does not trigger `SectionExpression` ownership first
- merged verification on 2026-03-08:
  - `./gradlew test --tests ch.njol.skript.lang.function.FunctionCoreCompatibilityTest --tests ch.njol.skript.lang.function.FunctionImplementationCompatibilityTest --tests ch.njol.skript.lang.function.FunctionCallCompatibilityTest --tests ch.njol.skript.registrations.ClassesCompatibilityTest --tests ch.njol.skript.lang.SkriptParserRegistryTest --tests ch.njol.skript.patterns.PatternCompilerCompatibilityTest --rerun-tasks`

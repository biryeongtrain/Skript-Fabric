...
- local `ch/njol/skript`: `305`
- local shortfall versus the captured upstream snapshot: `884`
...
| `classes` | `28` | `14` | partial shim | `P1` | foundational for parsing and stringification; the local tree now also restores legacy parser/converter wrappers plus Fabric-backed registry parser/classinfo/serializer shims, but the layer is still far thinner than upstream |
| `command` | `9` | `0` | absent | `P2` | command/runtime integration depends on core parser and function closure first |
| `conditions` | `135` | `7` | partial shim | `P2` | very large missing surface; after current dependency closure, import in larger bundles instead of one syntax family at a time |
...
| `effects` | `123` | `10` | partial shim | `P2` | base package now has a slightly wider verified effect surface through the new entity-state helpers, but most user-visible runtime forms are still missing |
...
| `expressions` | `391` | `23` | partial shim | `P2` | larger collection, string, and value helpers are now landing, but the remaining user-visible surface is still very large |
...
| `structures` | `10` | `5` | partial shim | `P1` | `options:` is real and the current batch now verifies `StructEvent` plus `StructExample`, but `StructFunction`, `StructUsing`, and broader structure closure are still open |
...
| --- | --- | --- | --- |
| Parser flow | `SkriptParser` now uses a shared compiled matcher, forwards general parse tags plus XOR marks, preserves duplicate parse tags in encounter order, keeps the current natural forms green, derives the current bare leading `:` auto-tags again, preserves placeholder-local `*` / `~` / `@time` metadata, no longer fails on omitted optional/alternation raw-regex captures, exposes a lightweight `PatternElement` graph through `SkriptPattern`, preserves grouped string/combinations parity plus malformed pattern wrapping on that graph, applies `Keyword` prefiltering again, backfills omitted non-optional placeholders from parser-scoped `DefaultValueData`, now resolves the most specific compatible parser/classinfo default for omitted placeholders, now restricts classinfo-backed omitted defaults to exact placeholder types, now ignores placeholders on inactive choice branches when deciding omitted-placeholder defaults, now retains the upstream-style error when only invalid omitted-placeholder defaults are available, now also fails the whole pattern when a required omitted placeholder has no default at all, requires exact parser-default type matches for omitted placeholders, runs legacy `parseStatic(...)` with `ALL_FLAGS` so expression-only placeholders work again through `SyntaxElementInfo`, rejects blank trimmed input before fully optional patterns can match, preserves placeholder whitespace in compiled pattern matches, recognizes upstream-prefixed variable forms such as `var {x}` / `variable {x}` / `the variable {x}`, allows upstream-style bare literal fallback through `InputSource`, keeps `ParserInstance.isCurrentEvent(...)` on upstream's one-way subtype rule, and now clears transient parser bridge state when switching scripts; broader upstream pattern element-graph/runtime parity plus fuller default-value parity are still open | parser behavior controls every syntax import after this | `Part 1A` |
| Statement loading | `Statement.parse(...)` now retains specific function/effect/condition parse errors, lets later same-pattern plain statements win after earlier effect/condition init failures, rejects plain conditions used as section headers, clears inherited outer section ownership for plain statements, benefits from the first local hint-scope lifecycle, keeps earlier higher-quality effect/condition parse errors over later lower-quality plain-statement failures, now also preserves an earlier semantic parse error over a later lower-quality `NOT_AN_EXPRESSION` statement failure, restores upstream-style semantic default quality for plain severe parse errors, suppresses the generic fallback text when a more specific retained severe error already exists, resets effect-side and condition-side section ownership between parse candidates on section lines, restores the outer owner snapshot so nested section candidates stay safe on the real `.sk` path, drops stale section `SEVERE` diagnostics when statement fallback succeeds on the same section line, and now keeps `EffectSection` fallback in statement mode when section-mode init rejects the body; exact `set {_var} to true:` section ownership is regression-covered, but broader orchestration and built-in hint flow are still thin | statement ordering and section ownership determine real script semantics | `Part 1A` |
| Script loading | `ScriptLoader.replaceOptions(...)` is real now, `loadItems(...)` now handles registered section nodes before falling back to statements, plain effects can own section-managing expressions through `Effect.parse(...)`, section-versus-statement fallback now restores the better retained diagnostic, successful section-line statement fallback now also preserves retained non-default section diagnostics, stopping statements now emit unreachable-code warnings behind script-level warning suppression, nested section-contained stop-trigger intent now propagates through loader/runtime while `stopSection` stays local, section and temporary non-section hint scopes now open/freeze/merge through the active loader path, section lines that fall back successfully from `Section.parse(...)` to a plain statement now keep their narrowed hint scope, `loadItems(...)` now leaves `ParserInstance.getNode()` anchored at the loaded section root, retained severe fallback errors now stay semantic, renamed config nodes now resync parent lookup maps, and the active Fabric runtime parser strips inline comments plus `###` block comments through `Node.splitLine(...)`; broader loader/log/config hint flow is still incomplete | script preprocessing and trigger-item construction parity are still incomplete | `Part 1A` |
...
| Function runtime | `DynamicFunctionReference` now discriminates validation-cache entries by concrete parameter expressions, `FunctionRegistry.getSignature(...)` / `getFunction(...)` now prefer script-local matches before global fallback, exact non-`Object` parameter matches now beat broader assignable overloads during ambiguity resolution, overload resolution now stays ambiguous when different candidates only win exactness on different positions, keyed call arguments no longer leak caller-owned array mutations through `Classes.clone(...)`, `Parameter.newInstance(...)` now parses registered expressions as default values instead of accepting only literal `Classes.parse(...)` forms, preserves keyed argument metadata for ordinary keyed parameters, keeps keyed plural defaults on upstream's single-value-zip versus multi-value-unkeyed split, now preserves the source-script namespace for local dynamic references, invalidates namespace-only local dynamic references when `Functions.clearFunctions(...)` removes their script namespace, drops unresolved `from missing.sk` suffixes before global fallback, `Parameter.parse(...)` now preserves quoted/default subexpressions while rejecting duplicate parameter names under case-insensitive variable mode, `Function.execute(...)` now rejects direct over-arity calls again even for a single plural parameter, explicit empty argument slots are no longer replaced by optional defaults, direct `null` slots now pass the legacy `executeWithNulls` guard again, and `FunctionReference.parse(...)` now unescapes doubled quotes inside quoted literal arguments; broader namespace/default-parameter parity is still thinner than upstream | function calls are used by parser and trigger runtime alike | `Part 1A` |
| Variable runtime | `Variables` now covers case-insensitive storage, copy-back semantics, list-to-list reindexing, natural numeric ordering for prefix/list iteration, upstream-style raw nested map reads for `Variables.getVariable("list::*", ...)`, list deletion through `Variables.setVariable("name::*", null, ...)`, legacy list-variable loop/check semantics, a first parse-time local-variable hint path through `HintManager`, snapshot/rollback through `HintManager.backup()` and `restore(...)`, section-only scope clearing that removes the targeted section frame instead of leaking copied hints, and the legacy `TypeHints` compatibility bridge, but it is still an in-memory bridge only | variable semantics affect function calls, sections, and expressions | `Part 1B` |
| Type/parse registry | `Classes` now covers codename/literal/supertype lookup, stable class-info ordering, shared literal-match ordering, case-sensitive classinfo codename lookup, explicit-literal-only `getPatternInfos(...)` parity, converter-backed parse fallback, converter-backed `getParser(...)` fallback, parse-log-aware fallback cleanup in `Classes.parse(...)`, upstream helper default-expression lookup overloads, the upstream null sentinel for empty-array stringification, legacy parser-backed value stringification through `Classes.toString(...)` including debug `[codename:text]` wrapping, upstream `object:` fallback for parser-less variable-name stringification, classinfo-backed cloning through `Classes.clone(...)`, and the legacy parser/converter wrapper surface through `Parser`, `PatternedParser`, `Converter`, and `registrations.Converters`, but remains a small compatibility layer relative to upstream | typed literal and parser behavior depend on it | `Part 1B` |
| User-visible syntax imports | `Part 2` now covers base entity-state/control syntax plus follow-up `%material%`, `feed`, invisible/visible condition and effect forms, burning/on-fire forms, AI forms, sprinting condition/effect forms, interaction-dimensions plural expressions, responsive/unresponsive interaction condition syntax, and `make %entities% adult` / `make %entities% baby` on the active Fabric runtime, but most upstream condition/effect/expression families are still absent from the registered Fabric surface | syntax import remains important, but the worker plan is shifting to larger `ch/njol/skript` bundle closure before more polish-oriented syntax slices | `Part 2` |
...
- merged six verified worker bundles on 2026-03-09:
  - Lane A restored Fabric-backed `RegistryParser`, `RegistryClassInfo`, and `RegistrySerializer`
  - Lane B restored `Time`, `Timeperiod`, `Experience`, `GameruleValue`, and `ClassInfoReference`
  - Lane C restored the formerly blocked `StructEvent` and `StructExample` structure slice
  - Lane D restored `RedirectingLogHandler`, `TestingLogHandler`, and the parser current-structure bridge
  - Lane E restored the string/value expression bundle (`ExprJoinSplit`, `ExprIndicesOfValue`, `ExprDefaultValue`) plus `CondAlphanumeric`
  - Lane F widened the entity leaf registry again through `ClassEntityData`, the remaining upstream entity wrappers, and a small panda/screaming effect slice
- merged verification on 2026-03-09:
  - `./gradlew build --rerun-tasks`
- raw `ch/njol/skript` snapshot after that merge: `305 / 1189`, shortfall `884`

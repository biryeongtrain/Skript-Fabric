...
| Function runtime | `DynamicFunctionReference` now discriminates validation-cache entries by concrete parameter expressions, `FunctionRegistry.getSignature(...)` / `getFunction(...)` now prefer script-local matches before global fallback, exact non-`Object` parameter matches now beat broader assignable overloads during ambiguity resolution, keyed call arguments no longer leak caller-owned array mutations through `Classes.clone(...)`, and `Parameter.newInstance(...)` now parses registered expressions as default values instead of accepting only literal `Classes.parse(...)` forms; broader namespace/default-parameter parity is still thinner than upstream | function calls are used by parser and trigger runtime alike | `Part 1A` |
...
- restored case-sensitive `ClassInfo` codename lookup in `Classes`, so mixed-case probes no longer resolve lowercase-only registrations
- restored exact-type overload preference in `FunctionRegistry`, so compatible exact matches now beat broader assignable candidates instead of collapsing to `AMBIGUOUS`
- restored fail-fast parser behavior when a required placeholder is omitted with no parser-owned or classinfo default available, so `SkriptParser` rejects the pattern instead of carrying a `null` expression farther than upstream does

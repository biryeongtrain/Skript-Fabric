...
- function runtime now closes dynamic validation-cache discrimination, script-local-before-global registry lookup, parsed default expressions for function parameters, and upstream-style parameter-list splitting/duplicate-name rejection in `Parameter.parse(...)`, but broader namespace/default-parameter parity is still incomplete
- natural numeric ordering for list/prefix iteration, legacy list-variable loop aliases, all-values list-check semantics, parse-time local variable type hints, and section-only hint-scope clearing are now closed, and shared literal-pattern matches now preserve upstream registration order for explicit literal aliases, but broader `Variables` and class-registry runtime semantics are still incomplete

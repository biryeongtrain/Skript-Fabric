...
- latest `lang` core closure batch:
  - `FunctionRegistry.getSignature(...)` and `getFunction(...)` now prefer script-local matches before falling back to global candidates, so compatible global overloads no longer make valid local calls ambiguous
  - `HintManager.clearScope(level, true)` now removes the targeted section scope frame instead of only clearing its hint map, so removed section hints no longer leak back into later local-variable parsing
  - `Effect.parse(...)` now resets `Section.SectionContext` ownership between registered effect candidates on section lines, so an earlier failed section-claiming effect candidate cannot let a later literal effect parse a section line incorrectly
  - `ScriptLoader` now replays retained non-default section diagnostics when `Section.parse(...)` fails but `Statement.parse(...)` fallback succeeds, and `ParseLogHandler` now exposes raw retained entries for that comparison
  - focused regression coverage now locks the new function, variable-hint, effect-ownership, and section-diagnostic paths in `FunctionCoreCompatibilityTest`, `FunctionCallCompatibilityTest`, `VariableCompatibilityTest`, and `ScriptLoaderCompatibilityTest`
...
- the parser no longer regresses the currently verified natural-script inline optional/alternation forms, now forwards general tags/XOR marks through the shared matcher, derives the current bare leading `:` auto-tags again, no longer fails on omitted optional/alternation raw-regex captures, now exposes a lightweight `PatternElement` graph introspection API, now resolves compatible omitted-placeholder parser/classinfo defaults, and now restores upstream-style bare string literal fallback through `InputSource`; broader upstream pattern element-graph/runtime parity is still incomplete
- function runtime now closes dynamic validation-cache discrimination, script-local-before-global registry lookup, and parsed default expressions for function parameters, but broader namespace/default-parameter parity is still incomplete
- natural numeric ordering for list/prefix iteration, legacy list-variable loop aliases, all-values list-check semantics, parse-time local variable type hints, and section-only hint-scope clearing are now closed, and shared literal-pattern matches now follow stable class-info ordering too, but broader `Variables` and class-registry runtime semantics are still incomplete

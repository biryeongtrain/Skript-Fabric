...
If only two workers are available, run `Coordinator + Lane A + Lane B` first.
`Lane C` is the third lane because it overlaps least with the active parser-heavy closure.
...
- `Lane A` tends to consume both parser and config behavior in final statement/loading orchestration
...
- `Lane C`: continue `Part 1B` on `Variables`, `Classes`, and the remaining `config` / `structures` behavior gaps after the just-closed validator-backed `options:` slice

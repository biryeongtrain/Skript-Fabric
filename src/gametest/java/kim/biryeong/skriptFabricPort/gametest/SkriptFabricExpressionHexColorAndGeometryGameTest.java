...
public final class SkriptFabricExpressionHexColorAndGeometryGameTest extends AbstractSkriptFabricGameTestSupport {
    ...
    public void hexCodeExpressionExecutesRealScript(GameTestHelper helper) {
        ...
        runWithRuntimeLock(helper, () -> {
            ...
            Variables.clearAll();
            try {
                runtime.loadFromResource("skript/gametest/expression/hex-color-and-geometry/hex_code_records_value.sk");

                int executed = dispatch(runtime, helper);
                assertExecuted(helper, executed, "hex code expression");
                assertText(helper, "hexgeo::hex_code", "FF0000");
    ...
    public void colorFromHexCodeExpressionExecutesRealScript(GameTestHelper helper) {
        ...
        runWithRuntimeLock(helper, () -> {
            ...
            Variables.clearAll();
            try {
                runtime.loadFromResource("skript/gametest/expression/hex-color-and-geometry/color_from_hex_records_channels.sk");

                int executed = dispatch(runtime, helper);
                assertExecuted(helper, executed, "color from hex expression");
                assertColor(helper, "hexgeo::color_from_hex::rgb", 255, 187, 167);
                assertColor(helper, "hexgeo::color_from_hex::argb", 51, 102, 204);
                helper.assertBlockPresent(Blocks.EMERALD_BLOCK, PROOF_MARKER);
            } finally {
                runtime.clearScripts();
                Variables.clearAll();
    ...
    public void recursiveSizeExpressionExecutesRealScript(GameTestHelper helper) {
        ...
        runWithRuntimeLock(helper, () -> {
            ...
            Variables.clearAll();
            try {
                runtime.loadFromResource("skript/gametest/expression/hex-color-and-geometry/recursive_size_records_value.sk");

                int executed = dispatch(runtime, helper);
                assertExecuted(helper, executed, "recursive size expression");
                assertNumber(helper, "hexgeo::recursive_size::count", 4);
                helper.assertBlockPresent(Blocks.EMERALD_BLOCK, PROOF_MARKER);
            } finally {
                runtime.clearScripts();
                Variables.clearAll();
    ...
    public void blockSphereExpressionExecutesRealScript(GameTestHelper helper) {
        ...
        runWithRuntimeLock(helper, () -> {
            ...
            try {
                ...
                runtime.loadFromResource("skript/gametest/expression/hex-color-and-geometry/block_sphere_records_blocks.sk");

                int executed = dispatch(runtime, helper);
                assertExecuted(helper, executed, "block sphere expression");
                Collection<Object> entries = variableValues("hexgeo::block_sphere::entries::");
                helper.assertTrue(
                        entries.size() == 7,
    ...
    private static void ensureSupportRegistered() {
        ...
        Skript.registerEvent(GameTestHexColorAndGeometryEvent.class, "gametest hex color and geometry context");
        Skript.registerExpression(HexGeoCenterLocationExpression.class, FabricLocation.class, "hexgeo-center-location");
    }

    private static int dispatch(SkriptRuntime runtime, GameTestHelper helper) {
        return runtime.dispatch(new SkriptEvent(new HexColorAndGeometryHandle(), helper.getLevel().getServer(), helper.getLevel(), null));
    }
    ...
    private record HexColorAndGeometryHandle() {
    }

    public static final class GameTestHexColorAndGeometryEvent extends ch.njol.skript.lang.SkriptEvent {
        @Override
        ...
        public boolean check(SkriptEvent event) {
            return event.handle() instanceof HexColorAndGeometryHandle;
        }

        @Override
        public Class<?>[] getEventClasses() {
            return new Class<?>[]{HexColorAndGeometryHandle.class};
        }

        @Override
        public String toString(@Nullable SkriptEvent event, boolean debug) {
            return "gametest hex color and geometry context";
        }
    }

    public static final class HexGeoCenterLocationExpression extends SimpleExpression<FabricLocation> {
        @Override

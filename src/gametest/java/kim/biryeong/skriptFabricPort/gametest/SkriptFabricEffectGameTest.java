...
public final class SkriptFabricEffectGameTest extends AbstractSkriptFabricGameTestSupport {
    ...
    @GameTest
    public void teleportEffectExecutesRealScript(GameTestHelper helper) {
        Interaction interaction = createInteraction(helper, true);
        assertUseEntityScriptSetsMarker(
                helper,
                "skript/gametest/effect/teleport_entity_marks_block.sk",
                interaction,
                new BlockPos(9, 1, 0),
    ...
    @GameTest
    public void dropItemEffectExecutesRealScript(GameTestHelper helper) {
        runWithRuntimeLock(helper, () -> {
            SkriptRuntime runtime = SkriptRuntime.instance();
            runtime.clearScripts();
            runtime.loadFromResource("skript/gametest/effect/drop_item_marks_block.sk");
            helper.assertBlockPresent(Blocks.GOLD_BLOCK, new BlockPos(5, 1, 0));
            runtime.clearScripts();
        });
    }

    @GameTest
    public void sleepBatEffectExecutesRealScript(GameTestHelper helper) {
        Bat bat = EntityType.BAT.create(helper.getLevel(), EntitySpawnReason.TRIGGERED);
        bat.setPos(Vec3.atCenterOf(helper.absolutePos(new BlockPos(5, 2, 5))));
        helper.getLevel().addFreshEntity(bat);

        assertUseEntityScriptSetsMarker(
                helper,
                "skript/gametest/effect/sleep_bat_marks_block.sk",
                bat,
                new BlockPos(9, 1, 0),
                Blocks.RED_WOOL,
                () -> helper.assertTrue(
                        bat.isResting(),
                        Component.literal("Expected bat to be resting after sleep effect.")
                )
        );
    }

    @GameTest
    public void growTreeEffectExecutesRealScript(GameTestHelper helper) {
        runWithRuntimeLock(helper, () -> {
            SkriptRuntime runtime = SkriptRuntime.instance();
            runtime.clearScripts();
            // Place dirt below for tree to grow
            BlockPos dirtPos = helper.absolutePos(new BlockPos(2, 0, 2));
            helper.getLevel().setBlockAndUpdate(dirtPos, Blocks.DIRT.defaultBlockState());
            runtime.loadFromResource("skript/gametest/effect/grow_tree_marks_block.sk");
            helper.assertBlockPresent(Blocks.GOLD_BLOCK, new BlockPos(5, 1, 0));
            runtime.clearScripts();
            helper.succeed();
        });
    ...
    @GameTest
    public void setDamageEffectExecutesRealScript(GameTestHelper helper) {
        runWithRuntimeLock(helper, () -> {
            SkriptRuntime runtime = SkriptRuntime.instance();
            runtime.clearScripts();
            runtime.loadFromResource("skript/gametest/effect/set_damage_marks_block.sk");

            ServerPlayer player = helper.makeMockServerPlayerInLevel();
            BlockPos markerPos = helper.absolutePos(new BlockPos(9, 1, 0));
            player.teleportTo(markerPos.getX() + 0.5D, markerPos.getY() + 1.0D, markerPos.getZ() + 0.5D);
            ...
            helper.assertTrue(
                    allowed,
                    Component.literal("Expected set-damage script to allow the damage event (not cancel it).")
            );
            helper.assertBlockPresent(Blocks.LIME_CONCRETE, new BlockPos(9, 1, 0));
            runtime.clearScripts();
        });
    }

...
public final class SkriptRuntimeLifecycle {
    ...
    public static void register(SkriptScriptService scriptService, Logger logger) {
        ServerLifecycleEvents.SERVER_STARTED.register(server -> onServerStarted(scriptService, logger));
        ServerLifecycleEvents.SERVER_STOPPING.register(server -> onServerStopping(scriptService, logger));
    }

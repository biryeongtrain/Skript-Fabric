...
public record FunctionReferenceParser(ParseContext context, int flags) {
	...
	public <T> FunctionReference<T> parseFunctionReference(String expr) {
		try (ParseLogHandler log = SkriptLogger.startParseLogHandler()) {
			...
			Argument<String>[] arguments = new FunctionArgumentParser(args).getArguments();
	 ...
	 * @return A {@link FunctionReference} if a function is found, or {@code null} if none is found.
	 */
	public <T> FunctionReference<T> parseFunctionReference(String name, Argument<String>[] arguments, ParseLogHandler log) {
	 ...
	 */
	private void doesNotExist(String name, Argument<String>[] arguments, Set<Signature<?>> possibleSignatures) {
	 ...
	 */
	private ArgumentParseResult parseFunctionArguments(Argument<String>[] arguments, ArgumentParseTarget[] targets) {
		assert arguments.length == targets.length;
		assert Arrays.stream(targets).map(ArgumentParseTarget::type).noneMatch(Class::isArray);

		//noinspection unchecked
		Argument<Expression<?>>[] parsed = (Argument<Expression<?>>[])
				new Argument[arguments.length];

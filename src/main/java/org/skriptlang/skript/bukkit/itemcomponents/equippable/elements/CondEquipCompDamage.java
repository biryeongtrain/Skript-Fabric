...
public final class CondEquipCompDamage extends Condition {
    ...
    public boolean check(SkriptEvent event) {
        return values.check(event, value -> {
            ...
            return equippable != null && equippable.damageOnHurt();
        }, isNegated());
    }

    @Override
    public String toString(SkriptEvent event, boolean debug) {
        return values.toString(event, debug) + (isNegated() ? " will not" : " will") + " lose durability when injured";
    }
}

...
public final class CondEquipCompDispensable extends Condition {
    ...
    public boolean init(Expression<?>[] expressions, int matchedPattern, Kleenean isDelayed, ParseResult parseResult) {
        ...
        setNegated((matchedPattern % 2) == 1 || matchedPattern == 3);
        return true;
    }
    ...
    public boolean check(SkriptEvent event) {
        return values.check(event, value -> {
            ...
            return equippable != null && equippable.dispensable();
        }, isNegated());
    }

    @Override
    public String toString(SkriptEvent event, boolean debug) {
        return values.toString(event, debug) + (isNegated() ? " is not" : " is") + " able to be dispensed";
    }
}

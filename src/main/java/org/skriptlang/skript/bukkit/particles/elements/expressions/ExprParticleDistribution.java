...
public final class ExprParticleDistribution extends SimpleExpression<Vec3> {
    ...
    protected Vec3 @Nullable [] get(SkriptEvent event) {
        return particles.stream(event)
                .filter(ParticleEffect::isUsingNormalDistribution)
                .map(ParticleEffect::distribution)
                .toArray(Vec3[]::new);
    }
    ...
    public void change(SkriptEvent event, Object @Nullable [] delta, ChangeMode mode) {
        ...
        for (ParticleEffect effect : particles.getAll(event)) {
            Vec3 next = switch (mode) {
                ...
                case ADD -> effect.distribution().add(change);
                case REMOVE -> effect.distribution().subtract(change);
                default -> effect.distribution();
            };
            effect.distribution(next);
        }
    }
    ...
    @Override public String toString(@Nullable SkriptEvent event, boolean debug) { return "particle distribution of " + particles.toString(event, debug); }
}

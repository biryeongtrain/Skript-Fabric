...
public final class CondTextDisplaySeeThroughBlocks extends Condition {
    ...
    public boolean check(SkriptEvent event) {
        return displays.check(event, value -> value instanceof Display.TextDisplay textDisplay
                && (PrivateEntityAccess.textDisplayFlags(textDisplay) & Display.TextDisplay.FLAG_SEE_THROUGH) != 0, isNegated());
    }

    @Override
    public String toString(SkriptEvent event, boolean debug) {
        if (isNegated()) {
            return displays.toString(event, debug) + " is not visible through blocks";
        }
        return displays.toString(event, debug) + " is visible through blocks";
    }
}

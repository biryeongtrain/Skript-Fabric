...
public class ArithmeticChain<L, R, T> implements ArithmeticGettable<T> {
	...
	@Nullable
	@SuppressWarnings("unchecked")
	public T get(SkriptEvent event) {
	...
	public static <L, R, T> ArithmeticGettable<T> parse(List<?> chain) {
		...
		for (Set<?> group : operatorGroups) {
			int lastIndex = findLastIndex(chain, group::contains);
			if (lastIndex == -1)
				continue;
	...
	/**
	 * Finds the last index of an element in the list that matches the predicate.
	 * Inlined replacement for {@code Utils.findLastIndex()}.
	 */
	private static <E> int findLastIndex(List<E> list, java.util.function.Predicate<Object> predicate) {

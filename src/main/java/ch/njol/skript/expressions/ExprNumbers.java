...
public class ExprNumbers extends SimpleExpression<Number> {
    ...
    @Override
    protected @Nullable Number[] get(SkriptEvent event) {
        Number s = start.getSingle(event);
        Number f = end.getSingle(event);
        if (s == null || f == null) {
            return null;
        }
        ...
        if (mode == 0) {
            ...
            final int precision = decimalPrecision(reverse ? f : s);
    ...
    @Override
    public @Nullable Iterator<Number> iterator(SkriptEvent event) {
        Number s = start.getSingle(event);
        Number f = end.getSingle(event);
        if (s == null || f == null) {
            return null;
        ...
        return new Iterator<>() {
            ...
            final int precision = decimalPrecision(reverse ? finish : starting);
            final double multiplier = Math.pow(10, precision);
    ...
    private static int decimalPrecision(Number value) {

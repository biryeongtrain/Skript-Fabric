...
public class ExprFilter extends SimpleExpression<Object> implements InputSource, KeyProviderExpression<Object> {
    ...
    static {
        Skript.registerExpression(ExprFilter.class, Object.class,
    ...
    private final WeakHashMap<SkriptEvent, java.util.List<String>> cache = new WeakHashMap<>();
    ...
    @Override
    public @NotNull Iterator<?> iterator(SkriptEvent event) {
        if (keyed) {
    ...
    public Iterator<KeyedValue<Object>> keyedIterator(SkriptEvent event) {
        ...
        Iterator<KeyedValue<Object>> keyedIterator =
                ((KeyProviderExpression<Object>) unfilteredObjects).keyedIterator(event);
        return StreamSupport.stream(
                        Spliterators.spliteratorUnknownSize(keyedIterator, Spliterator.ORDERED),
                        false
                )
                .filter(keyedValue -> {

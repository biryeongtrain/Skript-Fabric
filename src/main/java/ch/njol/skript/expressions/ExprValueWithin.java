...
public class ExprValueWithin extends WrapperExpression<Object> implements KeyProviderExpression<Object> {
	...
	static {
		Skript.registerExpression(ExprValueWithin.class, Object.class, "[the] (%-*classinfo%|value[:s]) (within|in) %~objects%");
	}
	...
	public boolean init(Expression<?>[] exprs, int matchedPattern, Kleenean isDelayed, ParseResult parseResult) {
		...
		if (exprs[0] != null) {
			...
			plural = classInfoReference.getSingle(null).isPlural().isTrue();
		} else {
		...
		if (plural == exprs[1].isSingle()) {
			if (plural) {
				...
				Skript.error(exprs[1].toString(null, false) + " may contain more than one " + (classInfo == null ? "value" :  classInfo.getCodeName()));
			}
			return false;
		}

		classInfo = exprs[0] == null ? null : ((Literal<ClassInfo<?>>) exprs[0]).getSingle(null);
	...
	public String toString(@Nullable SkriptEvent event, boolean debug) {
		return (classInfo == null ? "value" : classInfo.getCodeName()) + " within " + getExpr();
	}

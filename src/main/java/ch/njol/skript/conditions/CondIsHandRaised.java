...
public class CondIsHandRaised extends Condition {
    ...
    public boolean check(SkriptEvent event) {
        return entities.check(
                event,
                livingEntity -> livingEntity.isUsingItem() && (hand == null || livingEntity.getUsedItemHand() == hand),
                isNegated()
        );
    }
    ...
    public String toString(@Nullable SkriptEvent event, boolean debug) {
        return entities.toString(event, debug)
                + "'s "
                + (hand == null ? "" : (hand == InteractionHand.MAIN_HAND ? "main " : "off "))
                + "hand"
                + (entities.isSingle() ? " is" : "s are")
                + (isNegated() ? " not " : " ")
                + "raised";
    }
}
